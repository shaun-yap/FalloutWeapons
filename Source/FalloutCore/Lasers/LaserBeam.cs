...
namespace FalloutCollaborationProject
{
    public class LaserBeam : Bullet
    {
        ...
        protected override void Impact(Thing hitThing)
        {
            ...
            if (pawn?.health?.hediffSet?.hediffs?.Where(h => h.def.HasModExtension<BarrelLength>()).Count() > 0)
            {
                var hediff = pawn?.health?.hediffSet?.hediffs?.Where(h => h.def.HasModExtension<BarrelLength>()).First();
                var options = hediff.def.GetModExtension<BarrelLength>();
                a = origin + dir * (options == null ? 0.9f : options.barrelLength);
            }
            else
            {
                a = origin + dir * (defWeapon == null ? 0.9f : defWeapon.barrelLength);
            }
