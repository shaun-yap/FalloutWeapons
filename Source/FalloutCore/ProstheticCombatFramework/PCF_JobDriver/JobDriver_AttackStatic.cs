...
namespace OrenoPCF
{
    public class JobDriver_AttackStaticExtended : JobDriver
    {
        ...
        public bool TryStartAttack(LocalTargetInfo targ)
        {
            ...
            Verb verb = this.job.verbToUse;
            if (verb != null && verb.TryStartCastOn(targ, false, true))
            {
                Log.Message(verb + " - " + verb.DirectOwner);
                if (verb is Verb_ShootOneUse && verb.verbTracker.directOwner is HediffComp_VerbGiverExtended hediffComp)
                {
                    var hediff = hediffComp.parent as Hediff;
